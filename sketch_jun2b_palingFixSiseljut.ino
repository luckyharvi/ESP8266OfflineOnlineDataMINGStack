...
const long RAW_PTS[] = { 144000, 173500, 203000, 232500 };
const float CM_PTS[]  = {      0.0,      1.0,     2.0,     3.0 };
const int  N_PTS     = 4;

// After 15 cm (RAW ≥ 1970000), every +270000 raw → +5 cm
const long  RAW_STEP = 29633;
const float CM_STEP  = 1.0;
...
void loop() {
  ...
  if (WiFi.status() != WL_CONNECTED && digitalRead(MOSFET_PIN) != HIGH) {
    digitalWrite(LED_BUILTIN, HIGH);
...
void handleRtcMosfetSchedule() {
  ...
  if ((currentMinute >= 0 && currentMinute < 10) || (currentMinute >= 30 && currentMinute < 40)) {
    // Turn Wi-Fi ON
  ...
  } else if ((currentMinute >= 10 && currentMinute < 30) || (currentMinute >= 40 && currentMinute < 60)) {
    // Turn Wi-Fi OFF
